...
\hline
\rule[-3.5cm]{0pt}{3.5cm} & \begin{itemize}[leftmargin=1em,itemsep=1pt,topsep=1pt,parsep=0pt]
...
\hline
\textbf{3 Model}\\[4pt]
Two classifiers are evaluated across six feature configurations and two task variants, producing four runs in total. The classifiers are logistic regression and a shallow decision tree. Both use the same scikit-learn pipeline, fitted to the balanced training set and evaluated on the validation split. The pipeline is shown in Figure 1. & \\[2pt]
\hline
\rule[-3.5cm]{0pt}{3.5cm} & \begin{itemize}[leftmargin=1em,itemsep=1pt,topsep=1pt,parsep=0pt]
...
Six feature configurations are tested. They are designed to progress from simple token-count representations through to richer, more linguistically informed features, so that each step in complexity can be assessed against the baseline. & \\[2pt]
\hline
\rule[-3.5cm]{0pt}{3.5cm} & \begin{itemize}[leftmargin=1em,itemsep=1pt,topsep=1pt,parsep=0pt]
...
The binary task removes neutral tweets and asks the classifier to distinguish negative from positive only. If the 3-class difficulty is concentrated at the neutral boundary, performance should improve substantially here. I expected a significant gain. & \\[2pt]
\hline
\rule[-3.5cm]{0pt}{3.5cm} & \begin{itemize}[leftmargin=1em,itemsep=1pt,topsep=1pt,parsep=0pt]
...
The central question of this project is not simply how good the classifier is, but what classifier performance tells us about the labels. The two are not the same thing. Not even close. & \\[2pt]
\hline
\rule[-3.5cm]{0pt}{3.5cm} & \begin{itemize}[leftmargin=1em,itemsep=1pt,topsep=1pt,parsep=0pt]
  \item No first-person --- absence of I/my
\end{itemize} \\
...
Notebook section 4.2 produces a systematic breakdown of model predictions by true and predicted class. Examining the misclassified tweets gives a ground-level view of where the model fails, and why. & \\[2pt]
\hline
\rule[-3.5cm]{0pt}{3.5cm} & \begin{itemize}[leftmargin=1em,itemsep=1pt,topsep=1pt,parsep=0pt]
  \item No first-person --- absence of I/my
\end{itemize} \\
