...
A classifier that performs  on neutral tweets may be revealing that the boundary 
between neutral and mildly positive or mildly negative as ambiguous.
Different annotators may draw the line differently,
 e.g, low F1 may be evidence of noisy labels. Both may be true at once.

THree smaller questions guide this analysis:
...
Model comparison to a benchmark then includes this systematic noise. 
 ...
 I think being explicit about label quality mattecrs.
...
The labels came from Amazon Mechanical Turk\footnote{Amazon Mechanical Turk (MTurk) is a crowdsourcing platform where requesters post small tasks and workers complete them for small payments, typically a few cents each.}. Workers were paid small amounts to read each tweet and click a sentiment label \citep{rosenthal2017semeval}. No expertise required. Each tweet receives at least three annotations. The majority label stands. Annotators disagree---sometimes the final label reflects little more than a coin toss---and when I look at the data closely, the so-called ``ground truth'' starts to feel less certain:
...
\begin{itemize}
  \item logistic regression: \texttt{max\_iter=5000}, \texttt{random\_state=82171165};
  \item decision tree: \texttt{max\_depth=3}, \texttt{random\_state=82171165}.
\end{itemize}
...
I made the tree shallow on purpose. I wanted to understand what the model latches onto more than getting the highest score. The \texttt{max\_depth=3} setting is as much a diagnostic tool as a classifier (see Section 5).
